...
export default function AccountPanel({ lang, member, onClose, onLogout }: Props) {
  ...
  const [activeTab, setActiveTab] = useState<"account" | "visitors">("account");
      ...
      <section className="member-panel account-panel" role="dialog" aria-modal="true" onMouseDown={(e) => e.stopPropagation()}>
        ...
        {isOwner ? (
          <div className="account-tabs">
            <button
              type="button"
              className={`account-tab ${activeTab === "account" ? "is-active" : ""}`}
            ...
            <button
              type="button"
              className={`account-tab ${activeTab === "visitors" ? "is-active" : ""}`}
              onClick={() => setActiveTab("visitors")}
            >
              {fr ? "Visiteurs" : "Visitors"}
            </button>
          </div>
        ) : null}

        {isOwner && activeTab === "visitors" ? <VisitorsTab lang={lang} /> : null}

        {(!isOwner || activeTab === "account") ? (
          <>
            ...
            <div className="account-add">
              <input value={favorite} onChange={(e) => setFavorite(e.target.value)} placeholder={fr ? "Lieu, navire, avion…" : "Place, ship, aircraft…"} />
              <button onClick={() => { if (favorite.trim()) { void save({ ...preferences, favorites: [...preferences.favorites, { name: favorite.trim() }] }); setFavorite(""); } }}>+</button>
            </div>
            <div className="account-chips">
              {preferences.favorites.map((item, index) => (
                <button key={`${item.name}-${index}`} onClick={() => void save({ ...preferences, favorites: preferences.favorites.filter((_, i) => i !== index) })}>{item.name} ×</button>
              ))}
            </div>

            <h3>{fr ? "Mes alertes personnalisées" : "My custom alerts"}</h3>
            <div className="account-add">
              <input value={alertName} onChange={(e) => setAlertName(e.target.value)} placeholder={fr ? "Ex. cargo près de Montréal" : "E.g. cargo near Montreal"} />
              <button onClick={() => { if (alertName.trim()) { void save({ ...preferences, alerts: [...preferences.alerts, { name: alertName.trim(), enabled: true }] }); setAlertName(""); } }}>+</button>
            </div>
            <div className="account-alerts">
              {preferences.alerts.map((item, index) => (
                <label key={`${item.name}-${index}`}>
                  <input type="checkbox" checked={item.enabled} onChange={(e) => { const alerts = [...preferences.alerts]; alerts[index] = { ...item, enabled: e.target.checked }; void save({ ...preferences, alerts }); }} />
                  {item.name}
                  <button onClick={() => void save({ ...preferences, alerts: preferences.alerts.filter((_, i) => i !== index) })}>×</button>
                </label>
              ))}
            </div>
